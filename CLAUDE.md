...
docker build -t futu-opend-docker \
  --build-arg FUTU_OPEND_VER=10.7.6728 \
...
bash script/download_futu_opend.sh Futu_OpenD_10.7.6728_Ubuntu18.04.tar.gz
...
2. **`script/start.sh`** — runtime brain. Generates `/tmp/FutuOpenD.xml` from a heredoc, interpolating env vars (`FUTU_OPEND_IP`, `FUTU_OPEND_PORT`, `FUTU_ACCOUNT_ID`, `FUTU_OPEND_LANG`, `FUTU_OPEND_LOG_LEVEL`, etc.) — there is no embedded template file. MD5-hashes `FUTU_ACCOUNT_PWD` if `FUTU_ACCOUNT_PWD_MD5` is unset (and emits a stderr deprecation warning when only the plaintext is set), conditionally includes the telnet block when `FUTU_OPEND_TELNET_PORT` is set and the WebSocket block when `FUTU_OPEND_WEBSOCKET_PORT` is set, then launches `/bin/FutuOpenD -cfg_file=/tmp/FutuOpenD.xml` as a child process (no `exec` builtin — bash stays as PID 1, which means signals to the container are not forwarded to FutuOpenD).
