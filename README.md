...
```bash
docker build -t futu-opend-docker --build-arg FUTU_OPEND_VER=10.5.6508 --build-arg BASE_IMG=ubuntu .
```

- Use centos as base image

```bash
docker build -t futu-opend-docker --build-arg FUTU_OPEND_VER=10.5.6508 --build-arg BASE_IMG=centos .
```
